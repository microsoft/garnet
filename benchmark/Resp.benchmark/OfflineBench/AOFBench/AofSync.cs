...
namespace Resp.benchmark
{
    internal sealed class AofSync : IBulkLogEntryConsumer, IDisposable
    {
        ...
        public async Task Run(AofGen aofGen)
        {
            try
            {
                var iter = aofGen.log.ScanSingle(startAddress, long.MaxValue, scanUncommitted: true, recover: false, logger: logger);

                while (true)
                {
                    if (cts.Token.IsCancellationRequested) break;
                    await iter.BulkConsumeAllAsync(this, throttleMs: 0, maxChunkSize: maxChunkSize, cts.Token);
                }
            }
            catch (Exception ex)
            {
        ...
        public void RunEnqueue(AofGen aofGen)
        {
