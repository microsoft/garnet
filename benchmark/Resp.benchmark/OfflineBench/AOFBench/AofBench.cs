...
namespace Resp.benchmark
{
    public class AofBench
    {
        ...
        public AofBench(Options options)
        {
            ...
            sessions = server.GetRespSessions(options.NumThreads.Max());
            aofGen = new AofGen(0, options, stats);
            aofSync = new AofSync(this, threadId: 0, startAddress: 0, options, aofGen, stats);
        }
            ...
            unsafe void RunAOFBench(int threadId)
            {
                ...
                while (!done)
                {
                    ...
                    fixed (byte* payloadPtr = currPage.payload)
                    {
                        ...
                        aofSync.Consume(payloadPtr, currPage.payloadLength, currentAddress, nextAddress, isProtected: false);
