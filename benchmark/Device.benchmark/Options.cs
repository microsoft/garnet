...
namespace Device.benchmark
{
    public class Options
    {
        ...
        [Option("completion-threads", Required = false, Default = 0, HelpText = "Number of background drainer threads that wait on IO completions (0 = processor count on Windows, 1 on Linux). On Linux Native, each drainer is bound 1:1 to its own kernel io_context (libaio) or io_uring ring; submitters distribute across rings via per-thread affinity. For io_uring, throughput scales with this value up to the available submitter concurrency (and the uring backend uses min 4 rings even at value=1, with the single drainer covering all rings via the legacy QueueRun compat scanner). For libaio extra drainers rarely help past 1 because the kernel's per-context mutex is already efficient.")]
