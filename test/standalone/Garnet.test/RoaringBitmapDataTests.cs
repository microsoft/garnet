...
namespace Garnet.test.RoaringBitmap
{
    ...
    public class RoaringBitmapDataTests : TestBase
    {
        ...
        {
            var rb = new GarnetRoaringBitmap.RoaringBitmap();
            ClassicAssert.IsTrue(rb.IsEmpty);
            ClassicAssert.AreEqual(0, rb.Cardinality);
            ClassicAssert.AreEqual(0, rb.ChunkCount);
            ClassicAssert.IsFalse(rb.GetBit(0));
            ClassicAssert.IsFalse(rb.GetBit(uint.MaxValue));
            ClassicAssert.AreEqual(-1L, rb.BitPos(true));
            ClassicAssert.AreEqual(0L, rb.BitPos(false)); // first unset is at 0
        }
        ...
        {
            var rb = new GarnetRoaringBitmap.RoaringBitmap();
            ClassicAssert.IsFalse(rb.SetBit(42, true));
            ClassicAssert.IsTrue(rb.SetBit(42, true));   // already set
            ClassicAssert.IsTrue(rb.GetBit(42));
            ClassicAssert.AreEqual(1L, rb.Cardinality);
            ClassicAssert.IsTrue(rb.SetBit(42, false));   // clear returns previous=true
            ClassicAssert.IsFalse(rb.SetBit(42, false));  // clear non-existing
        ...
        public void ChunkBoundaryOffsets_AllRoundTrip()
        {
            ...
            foreach (var o in offsets) ClassicAssert.IsTrue(rb.GetBit(o), $"offset {o} should be set");
        ...
        public void RandomDenseSingleChunk_MatchesOracle()
        {
            ...
            {
                var v = (uint)rng.Next(0, 65536);
                var set = rng.Next(2) == 0;
                rb.SetBit(v, set);
        ...
        public void SparseAcrossChunks_MatchesOracle()
        {
            ...
            {
                var v = (uint)rng.NextInt64(0, uint.MaxValue);
                var set = rng.Next(4) != 0; // bias to set
                rb.SetBit(v, set);
                if (set) oracle.Add(v); else oracle.Remove(v);
            }
            AssertEqualsOracle(rb, oracle);
        ...
        public void SmallUniverse_BitPos_MatchesOracle()
        {
            ...
            for (var i = 0; i < Universe / 4; i++)
            {
                var v = rng.Next(Universe);
                bits[v] = true;
            ...
            {
                var from = rng.Next(Universe + 100);
                long expected1 = -1, expected0 = -1;
                for (var j = from; j < Universe; j++)
                {
                    if (expected1 < 0 && bits[j]) expected1 = j;
                    if (expected0 < 0 && !bits[j]) expected0 = j;
                ...
                var actual1 = rb.BitPos(true, (uint)from);
                var actual0 = rb.BitPos(false, (uint)from);
        ...
        public void BitPos_OnlyBit0_Set()
        {
            var rb = new GarnetRoaringBitmap.RoaringBitmap();
            rb.SetBit(0, true);
            ClassicAssert.AreEqual(0L, rb.BitPos(true));
            ClassicAssert.AreEqual(1L, rb.BitPos(false));
        }
        ...
        public void BitPos_FullFirstChunk_FindsZeroInSecond()
        {
            ...
            ClassicAssert.AreEqual(0L, rb.BitPos(true));
            ClassicAssert.AreEqual(65536L, rb.BitPos(false));
        }
        ...
        public void BitPos_GapBetweenChunks_FindsGap()
        {
            ...
            ClassicAssert.AreEqual(65536L, rb.BitPos(false));
            ClassicAssert.AreEqual(2 * 65536L, rb.BitPos(false, 2 * 65536u));
            // First set starting at 70000: look in chunk 1 (no chunk allocated, so skip)
            // and find the bit in chunk 2 at hi*65536+100.
            ClassicAssert.AreEqual(2 * 65536L + 100L, rb.BitPos(true, 70000u));
        }
        ...
        public void BitPos_MaxValue_Set()
        {
            ...
            ClassicAssert.AreEqual(uint.MaxValue, rb.BitPos(true));
            ClassicAssert.AreEqual(0L, rb.BitPos(false));
            ClassicAssert.AreEqual(uint.MaxValue - 1, rb.BitPos(false, uint.MaxValue - 1));
            // From uint.MaxValue with bit==false — that one IS set, so no unset >= MaxValue.
            ClassicAssert.AreEqual(-1L, rb.BitPos(false, uint.MaxValue));
        }
        ...
        public void BitPos_NoSetBits_ReturnsMinusOne()
        {
            var rb = new GarnetRoaringBitmap.RoaringBitmap();
            ClassicAssert.AreEqual(-1L, rb.BitPos(true));
            ClassicAssert.AreEqual(-1L, rb.BitPos(true, 1234));
        }
        ...
        public void Serialize_RoundTrip_Matches(int count)
        {
            ...
            {
                var rb2 = GarnetRoaringBitmap.RoaringBitmap.Deserialize(br);
        ...
        public void Deserialize_BadVersion_Throws()
        {
            ...
            using var ms = new MemoryStream(bytes);
            using var br = new BinaryReader(ms);
            ClassicAssert.Throws<InvalidDataException>(() => GarnetRoaringBitmap.RoaringBitmap.Deserialize(br));
        }
        ...
        private static void AssertEqualsOracle(GarnetRoaringBitmap.RoaringBitmap rb, HashSet<uint> oracle)
        {
            ...
            foreach (var v in oracle)
                ClassicAssert.IsTrue(rb.GetBit(v), $"oracle has {v}, rb does not");
            // Sample the universe to ensure rb has no extras.
            var rng = new Random(123);
            for (var i = 0; i < 5_000; i++)
            {
                var v = (uint)rng.NextInt64(0, uint.MaxValue);
                ClassicAssert.AreEqual(oracle.Contains(v), rb.GetBit(v), $"membership mismatch for {v}");
            }
