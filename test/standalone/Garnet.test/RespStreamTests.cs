...
namespace Garnet.test
{
    ...
    public class RespStreamTests : TestBase
    {
        ...
        public void StreamAutoGeneratedIdsSurviveAofReplay()
        {
            // XADD with '*' resolves the id from the wall clock, so the resolved id must be frozen
            // into the AOF record; otherwise replay regenerates different ids. Add entries with
            // '*', capture the returned ids, COMMITAOF, restart with recovery, and confirm the ids
            // are byte-for-byte identical after AOF replay.
            server.Dispose();
            TestUtils.DeleteDirectory(TestUtils.MethodTestDir);

            const string streamKey = "aofAutoId";
            const int count = 50;
            ...
            using (var redis = ConnectionMultiplexer.Connect(TestUtils.GetConfig(allowAdmin: true)))
            {
                ...
                {
                    var id = db.StreamAdd(streamKey, $"f{i}", $"v{i}"); // '*' auto-generated id
                    addedIds.Add(id.ToString());
                }
                // A 'ms-*' (explicit ms, auto sequence) entry on the same ms as the last id, to
                // also cover the auto-sequence freeze path.
                var lastMs = addedIds[^1].Split('-')[0];
                var autoSeqId = (string)db.Execute("XADD", streamKey, $"{lastMs}-*", "fa", "va");
                addedIds.Add(autoSeqId);
            ...
            {
                var db = redis.GetDatabase(0);
                ClassicAssert.AreEqual(addedIds.Count, db.StreamLength(streamKey),
                    "recovered length must match the number of entries added before restart");

                var range = db.StreamRange(streamKey, "-", "+");
                ClassicAssert.AreEqual(addedIds.Count, range.Length);
                for (int i = 0; i < addedIds.Count; i++)
                {
                    ClassicAssert.AreEqual(addedIds[i], range[i].Id.ToString(),
                        $"id at index {i} changed after AOF replay (auto-generated id was not frozen)");
                }
            }
        }
        ...
        [Test]
        public void XReadBlockOptionReturnsError()
        {
            using var redis = ConnectionMultiplexer.Connect(TestUtils.GetConfig());
            var db = redis.GetDatabase(0);
            db.StreamAdd("blk", "f", "v");

            // BLOCK is not supported and must be rejected rather than silently ignored.
            var ex = Assert.Throws<RedisServerException>(
                () => db.Execute("XREAD", "BLOCK", "0", "STREAMS", "blk", "0"));
            StringAssert.Contains("BLOCK is not supported", ex.Message);
        }

        [Test]
        public void XReadGroupBlockOptionReturnsError()
        {
            using var redis = ConnectionMultiplexer.Connect(TestUtils.GetConfig());
            var db = redis.GetDatabase(0);
            db.StreamAdd("blkg", "f", "v");
            db.Execute("XGROUP", "CREATE", "blkg", "grp", "0");

            var ex = Assert.Throws<RedisServerException>(
                () => db.Execute("XREADGROUP", "GROUP", "grp", "c1", "BLOCK", "0", "STREAMS", "blkg", ">"));
            StringAssert.Contains("BLOCK is not supported", ex.Message);
        }

        [Test]
        public void XLastIsNotARecognizedCommand()
        {
            using var redis = ConnectionMultiplexer.Connect(TestUtils.GetConfig());
            var db = redis.GetDatabase(0);
            db.StreamAdd("nolast", "f", "v");

            // XLAST was removed; the server must reject it as an unknown command.
            var ex = Assert.Throws<RedisServerException>(() => db.Execute("XLAST", "nolast"));
            StringAssert.Contains("unknown command", ex.Message.ToLowerInvariant());
        }
