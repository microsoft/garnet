...
namespace Garnet.test
{
    ...
    public class GarnetServerConfigTests
    {
        ...
        public void ImportExportConfigLocal()
        {
            ...
            var args = new[] { "--config-export-path", configPath, "-p", "4m", "-m", "128m", "-s", "2g", "--index", "128m", "--recover", "--port", "53", "--reviv-bin-record-count", "2", "--reviv-fraction", "0.5", "--reviv-bin-record-counts", "1,2,3", "--extension-bin-paths", string.Join(',', binPaths), "--loadmodulecs", string.Join(',', modules) };
            ...
            ClassicAssert.AreEqual(2, options.RevivBinRecordCounts);
