...
namespace Garnet.test
{
    ...
    public class GarnetServerConfigTests : AllureTestBase
    {
        ...
        public void AofSegmentSizeFlowsToTsavoriteLogSettings()
        {
            ...
            var serverOptions = options.GetServerOptions();
            serverOptions.GetAofSettings(0, null, out var logSettings);
            try
            {
                ClassicAssert.AreEqual(1L << 30, logSettings.SegmentSize);
            }
            finally
            {
            ...
            try
            {
                ClassicAssert.AreEqual(1L << 26, logSettings.SegmentSize);
            }
            finally
            {
                logSettings.LogDevice?.Dispose();
