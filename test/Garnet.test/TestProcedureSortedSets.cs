...
namespace Garnet
{
    ...
    sealed class TestProcedureSortedSets : CustomTransactionProcedure
    {
        ...
        {
            var offset = 0;
            var ssA = GetNextArg(ref parseState, ref offset);
        ...
        private static bool TestAPI<TGarnetApi>(TGarnetApi api, ref SessionParseState parseState) where TGarnetApi : IGarnetApi
        {
            ...
            {
                ssItems[i].score = GetNextArg(ref parseState, ref offset);
                ssItems[i].member = GetNextArg(ref parseState, ref offset);
                ssMembers[i] = ssItems[i].member;
            }

            var minRange = GetNextArg(ref parseState, ref offset);
            var maxRange = GetNextArg(ref parseState, ref offset);
            var match = GetNextArg(ref parseState, ref offset);
