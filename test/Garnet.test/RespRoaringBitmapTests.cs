...
namespace Garnet.test
{
    ...
    public class RespRoaringBitmapTests
    {
        ...
        {
            // Garnet stores strings in the main store and custom objects in the object
            // store. A key with the same name in both stores is a known property of
            // the two-store architecture, not a WRONGTYPE error path. Verify the
            // documented behaviour rather than asserting an exception.
            using var redis = ConnectionMultiplexer.Connect(TestUtils.GetConfig());
            var db = redis.GetDatabase(0);

            db.StringSet("rb", "hello");
            ClassicAssert.AreEqual(0, (long)db.Execute("R.SETBIT", "rb", "10", "1"));
            ClassicAssert.AreEqual("hello", (string)db.StringGet("rb"));
            ClassicAssert.AreEqual(1, (long)db.Execute("R.GETBIT", "rb", "10"));
        }
