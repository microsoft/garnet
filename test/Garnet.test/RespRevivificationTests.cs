...
namespace Garnet.test
{
    ...
    class RespRevivificationTests : AllureTestBase
    {
        ...
        public async Task RevivificationWithRmwWorksWhenNeedingShrinkingAndThenExpanding()
        {
            ...
            await db.ExecuteAsync("SETIFGREATER", "foo", "b", "1").ConfigureAwait(false);
        ...
        public async Task RevivificationWithRMWWorksViaSetIfGreater()
        {
            ...
            Func<IDatabase, Task> testRmwWorksViaSetIfGreater = async (db) =>
            {
                ...
                var res = (RedisResult[])await db.ExecuteAsync("SETIFGREATER", "foo", "c", "5").ConfigureAwait(false);
        ...
        public async Task RevivificationWithRMWWorksForRecordPool()
        {
            ...
            await db.ExecuteAsync("SET", "johnny", "x", "PX", 500).ConfigureAwait(false); // big record, that we need to make sure active exp picks up for revivification
            ...
            await db.ExecuteAsync("SETIFGREATER", "michael", "j", 23).ConfigureAwait(false);
            ...
            var res = (RedisResult[])await db.ExecuteAsync("GETWITHETAG", "michael").ConfigureAwait(false);
