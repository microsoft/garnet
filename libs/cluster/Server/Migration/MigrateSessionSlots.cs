    ...
    internal sealed unsafe partial class MigrateSession : IDisposable
    {
        ...
        public bool MigrateSlotsDriverInline()
        {
            ...
            void CreateAndRunMigrateTasks(StoreType storeType, long beginAddress, long tailAddress, int pageSize)
            {
                ...
                var migrateOperationRunner = new Task[clusterProvider.serverOptions.ParallelMigrateTasks];
                var i = 0;
                while (i < migrateOperationRunner.Length)
                {
                    var idx = i;
                    migrateOperationRunner[idx] = Task.Run(() => ScanStoreTask(idx, storeType, beginAddress, tailAddress, pageSize));
                    i++;
                }

                Task.WaitAll(migrateOperationRunner, _cts.Token);
            }
