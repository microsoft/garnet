...
namespace Garnet.cluster
{
    internal sealed partial class ReplicaSyncSession
    {
        ...
        {
            WaitForFlush().GetAwaiter().GetResult();
            AofSyncDriver.InitializeIfNeeded(isMainStore);
        }
        ...
        {
            WaitForFlush().GetAwaiter().GetResult();
            return AofSyncDriver.TryWriteKeyValueSpanByte(ref key, ref value, out task);
        }
        ...
        {
            WaitForFlush().GetAwaiter().GetResult();
            return AofSyncDriver.TryWriteKeyValueByteArray(key, value, expiration, out task);
        }
