...
namespace Garnet.cluster
{
    internal sealed partial class ReplicationManager : IDisposable
    {
        ...
        private async Task<string> InitiateReplicaSync()
        {
            ...
            try
            {
                gcs = new(address, port, new NetworkBuffers(1 << 12, 1 << 12).Allocate(logger: logger), clusterProvider.serverOptions.TlsOptions?.TlsClientOptions, authUsername: clusterProvider.ClusterUsername, authPassword: clusterProvider.ClusterPassword);
