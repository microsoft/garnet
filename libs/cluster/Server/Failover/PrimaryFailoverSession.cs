    ...
    internal sealed partial class FailoverSession : IDisposable
    {
        private async Task<long> CheckReplicaSync(GarnetClient gclient)
        {
            try
            {
                ...
                return await gclient.failreplicationoffset(clusterProvider.replicationManager.ReplicationOffset).WaitAsync(clusterTimeout, cts.Token).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
