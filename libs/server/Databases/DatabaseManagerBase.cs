    ...
    internal abstract class DatabaseManagerBase : IDatabaseManager
    {
            ...
            where TAllocator : IAllocator<TFuncs>
        {
            ...
            while (iter.GetNext())
            {
                ...
                else if (!basicContext.ContainsKeyInMemory(PinnedSpanByte.FromPinnedSpan(iter.Key), out long tempKeyAddress, fromAddr).Found || iter.CurrentAddress != tempKeyAddress)
                {
