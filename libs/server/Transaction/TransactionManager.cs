...
namespace Garnet.server
{
    ...
    public sealed unsafe partial class TransactionManager
    {
        ...
        internal void Log(byte id, ref CustomProcedureInput procInput)
        {
            ...
            appendOnlyFile?.Enqueue(new AofHeader(AofEntryType.StoredProcedure, id, basicContext.Session.Version, basicContext.Session.ID), ref procInput, out _);
        }
        ...
        internal void Commit(bool internal_txn = false)
        {
            ...
            {
                appendOnlyFile.Enqueue(new AofHeader(AofEntryType.TxnCommit, basicContext.Session.Version, basicContext.Session.ID), out _);
            }
            if (!internal_txn)
        ...
        internal bool Run(bool internal_txn = false, bool fail_fast_on_lock = false, TimeSpan lock_timeout = default)
        {
            ...
            if (appendOnlyFile != null && !functionsState.StoredProcMode)
            {
                appendOnlyFile.Enqueue(new AofHeader(AofEntryType.TxnStart, basicContext.Session.Version, basicContext.Session.ID), out _);
            }
