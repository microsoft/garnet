...
namespace Garnet.server
{
    ...
    public abstract class CustomTransactionProcedure : CustomFunctions
    {
        ...
        /// </summary>
        public abstract bool Prepare<TGarnetReadApi>(TGarnetReadApi api, ref SessionParseState parseState, int parseStateFirstArgIdx)
            where TGarnetReadApi : IGarnetReadApi;

        /// <summary>
        /// Main transaction: allowed to read and write (locks are already taken) and produce output
        /// </summary>
        public abstract void Main<TGarnetApi>(TGarnetApi api, ref SessionParseState parseState, int parseStateFirstArgIdx, ref MemoryResult<byte> output)
        ...
        /// </summary>
        public virtual void Finalize<TGarnetApi>(TGarnetApi api, ref SessionParseState parseState, int parseStateFirstArgIdx, ref MemoryResult<byte> output)
            where TGarnetApi : IGarnetApi
        { }
    }
}