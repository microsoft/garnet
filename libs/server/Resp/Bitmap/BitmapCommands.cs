...
namespace Garnet.server
{
    ...
    internal sealed unsafe partial class RespServerSession : ServerSessionBase
    {
            ...
            where TGarnetApi : IGarnetApi
        {
            ...
            while (currCount < endCount)
            {
                ...
                if (command.ToUpper().Equals("OVERFLOW"))
                {
                    ...
                    {
                        while (!RespWriteUtils.WriteGenericError($"Overflow type {overflowArg} not supported", ref dcurr, dend))
                            SendAndReset();
                        return true;
                    }
                    currCount += 2;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            while (!RespWriteUtils.WriteGenericError($"Bitfield command {command} not supported", ref dcurr, dend))
                                SendAndReset();
                            return true;
                        }
            ...
            where TGarnetApi : IGarnetApi
        {
            ...
            if (writeError)
            {
                while (!RespWriteUtils.WriteGenericError("BITFIELD_RO only supports the GET subcommand."u8, ref dcurr, dend))
                    SendAndReset();
                readHead = (int)(ptr - recvBufferPtr);
                return true;
            }
