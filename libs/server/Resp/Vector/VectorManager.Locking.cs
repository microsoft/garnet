...
namespace Garnet.server
{
    ...
    public sealed partial class VectorManager
    {
        ...
        /// </summary>
        internal ReadVectorLock ReadVectorIndex(StorageSession storageSession, ref PinnedSpanByte key, ref StringInput input, scoped Span<byte> indexSpan, out GarnetStatus status)
        {
            ...
            var keyHash = storageSession.unifiedBasicContext.GetKeyHash(key);
            ...
            while (true)
            {
                ...
                try
                {
                    readRes = storageSession.Read_MainStore(key, ref input, ref indexConfig, ref storageSession.stringBasicContext);
                ...
                {
                    if (PartiallyDeleted(indexConfig.ReadOnlySpan))
                    {
                    ...
                    needsRecreate = NeedsRecreate(indexConfig.ReadOnlySpan);
                }
                else
                {
                    needsRecreate = false;
                ...
                if (needsRecreate)
                {
                    ...
                    try
                    {
                        try
                        {
                            writeRes = storageSession.RMW_MainStore(key, ref input, ref indexConfig, ref storageSession.stringBasicContext);
        ...
        )
        {
            ...
            var keyHash = storageSession.unifiedBasicContext.GetKeyHash(key.ReadOnlySpan);
        ...
        /// </summary>
        private ExclusiveVectorLock AcquireExclusiveLocks(StorageSession storageSession, ref PinnedSpanByte key)
        {
            var keyHash = storageSession.unifiedBasicContext.GetKeyHash(key);
        ...
        /// </summary>
        internal ExclusiveVectorLock ReadForDeleteVectorIndex(StorageSession storageSession, ref PinnedSpanByte key, ref StringInput input, scoped Span<byte> indexSpan, out GarnetStatus status)
        {
            Debug.Assert(indexSpan.Length == IndexSizeBytes, "Insufficient space for index");
            ...
            try
            {
                status = storageSession.Read_MainStore(key, ref input, ref indexConfig, ref storageSession.stringBasicContext);
            }
            catch
            {
