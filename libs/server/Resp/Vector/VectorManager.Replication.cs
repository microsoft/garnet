...
namespace Garnet.server
{
    ...
    public sealed partial class VectorManager
    {
            ...
            Func<RespServerSession> obtainServerSession, ReadOnlySpan<byte> key, ref StringInput input)
        {
            ...
            else if (input.arg1 == VADDSetFlagsArg)
            {
                // Flag updates carry the new flags instead of the usual VADD payload and only touch the index record.
                // RENAME suppresses cleanup on the old key before deleting it, so a replica that skips this reclaims
                // the context that the renamed Vector Set still stores its elements under.
                //
                // This arrives as a VADD, so AofProcessor dispatches it without the drain it performs for every
                // other command. Adds for this Vector Set are still queued at this point and rewrite the index
                // record when they run, which would discard the flags written here, so drain them first.
                WaitForVectorOperationsToComplete();

                var flags = MemoryMarshal.Read<VectorSetFlags>(input.parseState.GetArgSliceByRef(0).ReadOnlySpan);

                SetFlags(key, flags, ref currentSession.stringBasicContext);
                return;
            }
