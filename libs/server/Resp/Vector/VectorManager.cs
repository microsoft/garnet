    ...
    public sealed partial class VectorManager : IDisposable
    {
        ...
        public void ReconcileRecoveredState(bool requireNoReservedContexts = false)
        {
            ...
            lock (this)
            {
                ...
                // concurrently and the scan is the authoritative census of live indexes. This replaces
                // whatever RecoveredVectorSetIndexKey collected, so only the scan's values are ever read.
        ...
        public void RecoveredVectorSetIndexKey<TSourceLogRecord>(ref TSourceLogRecord record) where TSourceLogRecord : ISourceLogRecord
        {
            ...
            // Only the key matters here; ReconcileRecoveredState rebuilds this map from a store scan before
            // reading any value, so there is nothing to be gained by computing the hash slot on this path
