...
namespace Garnet.server
{
    ...
    {
        private void HashGet(ref ObjectInput input, ref ObjectStoreOutput output, byte respProtocolVersion)
        {
        ...
        private void HashMultipleGet(ref ObjectInput input, ref ObjectStoreOutput output, byte respProtocolVersion)
        {
            using var writer = new RespMemoryWriter(respProtocolVersion, ref output.SpanByteAndMemory);
        ...
        private void HashGetAll(ref ObjectStoreOutput output, byte respProtocolVersion)
        {
            using var writer = new RespMemoryWriter(respProtocolVersion, ref output.SpanByteAndMemory);
        ...
        private void HashDelete(ref ObjectInput input, ref ObjectStoreOutput output)
        {
            for (var i = 0; i < input.parseState.Count; i++)
            {
                var key = GetByteSpanFromInput(ref input, i);
        ...
        private void HashLength(ref ObjectStoreOutput output)
        {
            output.Header.result1 = Count();
        }

        private void HashStrLength(ref ObjectInput input, ref ObjectStoreOutput output)
        {
            var key = GetByteSpanFromInput(ref input, 0);
            output.Header.result1 = TryGetValue(key, out var hashValue) ? hashValue.Length : 0;
        }

        private void HashExists(ref ObjectInput input, ref ObjectStoreOutput output)
        {
            var field = GetByteSpanFromInput(ref input, 0);
            output.Header.result1 = ContainsKey(field) ? 1 : 0;
        }

        private void HashRandomField(ref ObjectInput input, ref ObjectStoreOutput output, byte respProtocolVersion)
        {
        ...
        private void HashSet(ref ObjectInput input, ref ObjectStoreOutput output)
        {
            DeleteExpiredItems();
        ...
        private void HashCollect(ref ObjectInput input, ref ObjectStoreOutput output)
        {
            DeleteExpiredItems();
            output.Header.result1 = 1;
        }

        private void HashGetKeysOrValues(ref ObjectInput input, ref ObjectStoreOutput output, byte respProtocolVersion)
        {
            var count = Count();
        ...
        private void HashIncrement(ref ObjectInput input, ref ObjectStoreOutput output, byte respProtocolVersion)
        {
            var op = input.header.HashOp;
        ...
        [SkipLocalsInit] // avoid zeroing the stackalloc buffer
        private void HashIncrementFloat(ref ObjectInput input, ref ObjectStoreOutput output, byte respProtocolVersion)
        {
            var op = input.header.HashOp;
        ...
        private void HashExpire(ref ObjectInput input, ref ObjectStoreOutput output, byte respProtocolVersion)
        {
            using var writer = new RespMemoryWriter(respProtocolVersion, ref output.SpanByteAndMemory);
        ...
        private void HashTimeToLive(ref ObjectInput input, ref ObjectStoreOutput output, byte respProtocolVersion)
        {
            DeleteExpiredItems();
        ...
        private void HashPersist(ref ObjectInput input, ref ObjectStoreOutput output, byte respProtocolVersion)
        {
            DeleteExpiredItems();
