    ...
    public unsafe partial class SortedSetObject : GarnetObjectBase
    {
        ...
        private void SortedSetScore(byte* input, ref SpanByteAndMemory output)
        {
            ...
            try
            {
                ...
                {
                    while (!RespWriteUtils.WriteDoubleBulkString(score, ref curr, end))
                        ObjectUtils.ReallocateOutput(ref output, ref isMemory, ref ptr, ref ptrHandle, ref curr, ref end);
                }
                _output.bytesDone = 0;
                _output.countDone = 1;
        ...
        private void SortedSetIncrement(byte* input, int length, ref SpanByteAndMemory output)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // write the new score
                    while (!RespWriteUtils.WriteDoubleBulkString(sortedSetDict[memberByteArray], ref curr, end))
                        ObjectUtils.ReallocateOutput(ref output, ref isMemory, ref ptr, ref ptrHandle, ref curr, ref end);
                    countDone = 1;
                }
        ...
        private void PopMinOrMaxCount(byte* input, ref SpanByteAndMemory output, SortedSetOperation op)
        {
            ...
            try
            {
                ...
                while (count > 0)
                {
                    ...
                    while (!RespWriteUtils.WriteDoubleBulkString(max.Score, ref curr, end))
                        ObjectUtils.ReallocateOutput(ref output, ref isMemory, ref ptr, ref ptrHandle, ref curr, ref end);
