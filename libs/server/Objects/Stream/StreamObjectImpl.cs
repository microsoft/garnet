...
namespace Garnet.server
{
    ...
    {
        // Per-stream pinned scratch holding the most recently resolved "ms-seq" id as ASCII.
        // Reused for the XADD reply and to freeze an auto-generated id ('*'/'ms-*') into the
        // AOF-logged command so replay/replication reproduces the same id rather than generating
        // a fresh one from a different clock (see OperateXAdd / FreezeAutoGeneratedId).
        readonly byte[] resolvedIdAscii = GC.AllocateArray<byte>((NumUtils.MaximumFormatInt64Length * 2) + 1, pinned: true);

        /// <summary>
        /// Adds an entry or item to the stream
        /// </summary>
        /// <param name="idWasAutoGenerated">True if the client used a server-generation token
        /// ('*' or 'ms-*'); the caller must then freeze the resolved id into the logged command
        /// for deterministic AOF replay (see <see cref="FreezeAutoGeneratedId"/>).</param>
        /// <param name="resolvedIdLength">Number of ASCII bytes written to <c>resolvedIdAscii</c>.</param>
        public unsafe void AddEntry(PinnedSpanByte idSlice, int numPairs, ReadOnlySpan<byte> rawFieldValuePairs, ref SpanByteAndMemory output, byte respProtocolVersion, out bool idWasAutoGenerated, out int resolvedIdLength)
        {
            idWasAutoGenerated = false;
            resolvedIdLength = 0;
            ...
            // A leading '*' (full auto, which uses the wall clock and is non-deterministic) or a
            // trailing '*' ('ms-*', auto sequence) means the server generated part of the id. The
            // caller freezes the resolved id into the AOF-logged command in that case.
            var idSpan = idSlice.ReadOnlySpan;
            idWasAutoGenerated = idSpan.Length > 0 &&
                (idSpan[0] == (byte)'*' || idSpan[idSpan.Length - 1] == (byte)'*');
            ...
            // Format the resolved "ms-seq" into the per-stream pinned buffer; reused for the RESP
            // reply and, when the id was auto-generated, for the AOF id-freeze rewrite in OperateXAdd.
            ulong idMS = id.getMS();
            ulong idSeq = id.getSeq();
            var idAscii = new Span<byte>(resolvedIdAscii, 0, resolvedIdAscii.Length);
            int len = NumUtils.WriteInt64((long)idMS, idAscii);
            idAscii[len++] = (byte)'-';
            len += NumUtils.WriteInt64((long)idSeq, idAscii.Slice(len));
            resolvedIdLength = len;

            writer.WriteBulkString(idAscii.Slice(0, len));
