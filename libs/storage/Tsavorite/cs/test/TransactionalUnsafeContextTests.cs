...
namespace Tsavorite.test.TransactionalUnsafeContext
{
    ...
    internal class TransactionalUnsafeComparer : IKeyComparer<long>
    {
...
namespace Tsavorite.test.TransactionalUnsafeContext
{
    using LongAllocator = BlittableAllocator<long, long, StoreFunctions<long, long, TransactionalUnsafeComparer, DefaultRecordDisposer<long, long>>>;
    using LongStoreFunctions = StoreFunctions<long, long, TransactionalUnsafeComparer, DefaultRecordDisposer<long, long>>;
    ...
    internal class TransactionalUnsafeFunctions : SimpleSimpleFunctions<long, long>
    {
    ...
    internal struct BucketLockTracker
    {
        ...
        internal readonly void Increment(FixedLengthTransactionalKeyStruct<long> key) => Increment(ref key); // easier with 'foreach' because iteration vars can't be passed by 'ref'
        internal readonly void Increment(ref FixedLengthTransactionalKeyStruct<long> key)
        {
        ...
        internal readonly void Decrement(FixedLengthTransactionalKeyStruct<long> key) => Decrement(ref key);
        internal readonly void Decrement(ref FixedLengthTransactionalKeyStruct<long> key)
        {
            if (key.LockType == LockType.Exclusive)
                DecrementX(ref key);
            else
        ...
        internal readonly void IncrementX(ref FixedLengthTransactionalKeyStruct<long> key) => AddX(ref key, 1);
        internal readonly void DecrementX(ref FixedLengthTransactionalKeyStruct<long> key) => AddX(ref key, -1);
        internal readonly void IncrementS(ref FixedLengthTransactionalKeyStruct<long> key) => AddS(ref key, 1);
        internal readonly void DecrementS(ref FixedLengthTransactionalKeyStruct<long> key) => AddS(ref key, -1);

        private readonly void AddX(ref FixedLengthTransactionalKeyStruct<long> key, int addend)
        {
        ...
        private readonly void AddS(ref FixedLengthTransactionalKeyStruct<long> key, int addend)
        {
            if (!buckets.TryGetValue(key.KeyHash, out var counts))
                counts = default;
            counts.s += addend;
        ...
        internal readonly bool GetLockCounts(ref FixedLengthTransactionalKeyStruct<long> key, out (int x, int s) counts)
        {
    ...
    class TransactionalUnsafeContextTests
    {
        ...
        private ClientSession<long, long, long, long, Empty, TransactionalUnsafeFunctions, LongStoreFunctions, LongAllocator> session;
        private BasicContext<long, long, long, long, Empty, TransactionalUnsafeFunctions, LongStoreFunctions, LongAllocator> bContext;
        private IDevice log;
        ...
        void AssertIsLocked(FixedLengthTransactionalKeyStruct<long> key, bool xlock, bool slock)
            => OverflowBucketLockTableTests.AssertLockCounts(store, ref key, xlock, slock);
        void AssertIsLocked(ref FixedLengthTransactionalKeyStruct<long> key, bool xlock, bool slock)
            => OverflowBucketLockTableTests.AssertLockCounts(store, ref key, xlock, slock);
        ...
        static void ClearCountsOnError(ClientSession<long, long, long, long, Empty, TransactionalUnsafeFunctions, LongStoreFunctions, LongAllocator> luContext)
        {
        ...
        internal void AssertBucketLockCount(ref FixedLengthTransactionalKeyStruct<long> key, long expectedX, long expectedS) => OverflowBucketLockTableTests.AssertBucketLockCount(store, ref key, expectedX, expectedS);

        internal enum LockOperationType { Lock, Unlock }

        internal static IEnumerable<int> EnumActionKeyIndices<TKey>(FixedLengthTransactionalKeyStruct<TKey>[] keys, LockOperationType lockOpType)
        {
        ...
        public void ManualLockCollidingHashCodes([Values] UseSingleBucketComparer /* justToSignalSetup */ _)
        {
            ...
            var keys = new[]
            {
                new FixedLengthTransactionalKeyStruct<long>(101L, genHashCode(1), LockType.Exclusive, lContext),
                new FixedLengthTransactionalKeyStruct<long>(102L, genHashCode(2), LockType.Exclusive, lContext),
                new FixedLengthTransactionalKeyStruct<long>(103L, genHashCode(3), LockType.Exclusive, lContext),
            };
        ...
        public async Task TestShiftHeadAddressLUC([Values] CompletionSyncMode syncMode)
        {
            ...
            var keyVec = new FixedLengthTransactionalKeyStruct<long>[1];
            ...
            try
            {
                ...
                var lockKeys = Enumerable.Range(0, NumRecs).Select(ii => new FixedLengthTransactionalKeyStruct<long>(r.Next(RandRange), LockType.Shared, luContext)).ToArray();
                                              ...
                                              [Values(UpdateOp.Upsert, UpdateOp.RMW)] UpdateOp updateOp)
        {
            ...
            var keys = new[]
            {
                new FixedLengthTransactionalKeyStruct<long>(readKey24, LockType.Shared, luContext),      // Source, shared
                new FixedLengthTransactionalKeyStruct<long>(readKey51, LockType.Shared, luContext),      // Source, shared
                new FixedLengthTransactionalKeyStruct<long>(resultKey, LockType.Exclusive, luContext),   // Destination, exclusive
            };
...
#pragma warning restore IDE0060 // Remove unused parameter
        {
            ...
            var keyVec = new[] { new FixedLengthTransactionalKeyStruct<long>(resultKey, LockType.Exclusive, luContext) };
        ...
        public void StressManualLocks([Values(1, 8)] int numLockThreads, [Values(0, 1, 8)] int numOpThreads)
        {
            ...
            void runManualLockThread(int tid)
            {
                ...
                IEnumerable<FixedLengthTransactionalKeyStruct<long>> enumKeysToLock()
                {
                ...
                for (var iteration = 0; iteration < numIterations; ++iteration)
                {
                    ...
                    FixedLengthTransactionalKeyStruct<long>.Sort(keys, luContext);
                    luContext.Lock(keys);
                    luContext.Unlock(keys);
                }
        ...
        FixedLengthTransactionalKeyStruct<long> AddLockTableEntry<TFunctions>(TransactionalUnsafeContext<long, long, long, long, Empty, TFunctions, LongStoreFunctions, LongAllocator> luContext, long key)
            where TFunctions : ISessionFunctions<long, long, long, long, Empty>
        {
            var keyVec = new[] { new FixedLengthTransactionalKeyStruct<long>(key, LockType.Exclusive, luContext) };
            luContext.Lock(keyVec);
        ...
        void VerifyAndUnlockSplicedInKey<TFunctions>(TransactionalUnsafeContext<long, long, long, long, Empty, TFunctions, LongStoreFunctions, LongAllocator> luContext, long expectedKey)
            where TFunctions : ISessionFunctions<long, long, long, long, Empty>
        {
            ...
            var keyVec = new[] { new FixedLengthTransactionalKeyStruct<long>(expectedKey, LockType.Exclusive, luContext) };
            luContext.Unlock(keyVec);
        }
        ...
        public void VerifyCountAfterUpsertToTailTest([Values] ChainTests.RecordRegion recordRegion)
        {
            ...
            FixedLengthTransactionalKeyStruct<long> keyStruct = default;
            try
            {
        ...
        public void LockAndUnlockInLockTableOnlyTest()
        {
            ...
            FixedLengthTransactionalKeyStruct<long> createKey(long key) => new(key, (key & 1) == 0 ? LockType.Exclusive : LockType.Shared, luContext);
        ...
        public void VerifyCountAfterReadOnlyToUpdateRecordTest([Values] UpdateOp updateOp)
        {
            ...
            var keyVec = new[] { new FixedLengthTransactionalKeyStruct<long>(42, LockType.Exclusive, luContext) };
        ...
        public void LockNewRecordThenUnlockThenUpdateTest([Values] UpdateOp updateOp)
        {
            ...
            var lockKeyVec = new FixedLengthTransactionalKeyStruct<long>[1];
            ...
            void locker(int key)
            {
                ...
                try
                {
                    // Begin/EndTransactional are called outside this function; we could not EndTransactional in here as the lock lifetime is beyond that.
                    // (BeginTransactional's scope is the session; BeginUnsafe's scope is the thread. The session is still "mono-threaded" here because
        ...
        public void TryLockTimeSpanLimitTest()
        {
            ...
            var keyVec = new FixedLengthTransactionalKeyStruct<long>[]
            {
            ...
            var blockingVec = new FixedLengthTransactionalKeyStruct<long>[1];
        ...
        public void TryPromoteLockTimeSpanLimitTest()
        {
            ...
            var exclusiveVec = new FixedLengthTransactionalKeyStruct<long>[] { new(key, LockType.Exclusive, luContext) };
            var sharedVec = new FixedLengthTransactionalKeyStruct<long>[] { new(key, LockType.Shared, luContext) };
