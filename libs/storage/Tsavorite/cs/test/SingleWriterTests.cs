...
namespace Tsavorite.test.SingleWriter
{
    ...
    class SingleWriterTests
    {
        ...
        public void Setup()
        {
            ...
            LogSettings logSettings = new LogSettings { LogDevice = log, ObjectLogDevice = null, PageSizeBits = 12, MemorySizePages = 1 << 10, ReadCopyOptions = new(ReadCopyFrom.Device, ReadCopyTo.MainLog) };
            foreach (var arg in TestContext.CurrentContext.Test.Arguments)
            {
                if (arg is ReadCopyDestination dest)
                {
                    ...
                    {
                        logSettings.ReadCacheSettings = new() { PageSizeBits = 12, MemorySizePages = 1 << 10 };
    ...
    public class StructWithStringTests
    {
        ...
        public void Setup()
        {
            ...
            store = new TsavoriteKV<StructWithString, StructWithString>(1L << 20,
                new LogSettings { LogDevice = log, ObjectLogDevice = objlog, PageSizeBits = 10, MemorySizePages = 1 << 12, SegmentSizeBits = 16 },
