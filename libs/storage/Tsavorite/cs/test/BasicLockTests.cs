    ...
    public class BasicLockTests : AllureTestBase
    {
        ...
        public void Setup()
        {
            ...
            store = new(new()
            {
                IndexSize = 1L << 26,
                LogDevice = log
            }, StoreFunctions.Create(keyComparer, SpanByteRecordTriggers.Instance)
        ...
        public void FunctionsLockTest([Values(1, 20)] int numThreads)
        {
            ...
            for (long key = 0; key < NumRecords; key++)
            {
                ...
                long output = 0;
                ClassicAssert.IsFalse(bContext.Read(TestSpanByteKey.FromPinnedSpan(SpanByte.FromPinnedVariable(ref key)), ref output).IsPending);
