...
namespace Tsavorite.test.spanbyte
{
    ...
    /// <summary>
    /// End-to-end tests for the direct-VM (mmap/VirtualAlloc) hash index. Flips the process-global
    /// <see cref="NativeAllocatorInitializer.Enabled"/> flag, so this fixture is
    /// <see cref="NonParallelizableAttribute"/> and resets it in teardown.
    /// </summary>
    [TestFixture]
    [NonParallelizable]
    internal class NativeHashIndexTests : TestBase
    {
        ...
        public void Setup()
        {
            ...
            GC.Collect();
            _ = NativeAllocatorInitializer.Initialize(true);
        }

        [TearDown]
        public void TearDown()
        {
            _ = NativeAllocatorInitializer.Initialize(false);
            GC.Collect();
            GC.WaitForPendingFinalizers();
            GC.Collect();
        ...
        public async Task NativeIndexGrowFreesSupersededTables()
        {
            ...
            // (there is no concurrent index checkpoint here, so the direct-VM owner's outstanding-IO count is 0 and none are deferred).
            ...
            // not the geometric sum of all generations. (Loose bound: DirectVmBytes may include unrelated direct-VM
            // allocations, so this is a coarse ceiling; the free-count delta above is the precise regression guard.)
        ...
        /// <summary>
        /// The index-checkpoint IO-unit release guard must be claimable exactly once even though
        /// <see cref="HashIndexPageAsyncFlushResult"/> is a struct that gets boxed when handed to the device as the
        /// completion callback's context: the issuer's copy and the callback's copy are distinct structs, so the guard
        /// has to be a reference type they share. Both the boxed and the unboxed copy must see the same claim, or a
        /// device that completes the callback synchronously and then throws out of the submit would release the unit
        /// twice and underflow the outstanding-IO counter (which gates freeing superseded native hash tables).
        /// </summary>
        [Test]
        [Category(TsavoriteKVTestCategory)]
        public void IndexCheckpointIoUnitReleaseIsExactlyOnce()
        {
            HashIndexPageAsyncFlushResult result = default;
            result.chunkIndex = 3;
            result.ioUnitReleaseGuard = new(0);

            // Emulate the device handing a boxed copy to the callback while the issuer keeps its own copy.
            object boxed = result;
            var callbackCopy = (HashIndexPageAsyncFlushResult)boxed;

            ClassicAssert.IsTrue(callbackCopy.TryClaimIoUnitRelease(), "first claim (callback copy) must win");
            ClassicAssert.IsFalse(result.TryClaimIoUnitRelease(),
                "issuer copy must observe the callback copy's claim through the shared guard");
            ClassicAssert.IsFalse(((HashIndexPageAsyncFlushResult)boxed).TryClaimIoUnitRelease(),
                "re-unboxing must not resurrect the claim");

            // Racing claimants: exactly one wins regardless of interleaving.
            HashIndexPageAsyncFlushResult raced = default;
            raced.ioUnitReleaseGuard = new(0);
            object racedBoxed = raced;
            var winners = 0;
            ...
            {
                var useBoxed = (ii & 1) == 0;
                threads[ii] = new System.Threading.Thread(() =>
                {
                    var copy = useBoxed ? (HashIndexPageAsyncFlushResult)racedBoxed : raced;
                    start.Wait();
                    if (copy.TryClaimIoUnitRelease())
                        _ = System.Threading.Interlocked.Increment(ref winners);
                });
                threads[ii].Start();
            }
            start.Set();
            foreach (var t in threads)
                t.Join();
            ClassicAssert.AreEqual(1, winners, "exactly one claimant must release the IO unit");
        }
    }
}