...
namespace Tsavorite.test.spanbyte
{
    ...
    /// <summary>
    /// Parametrizes a real store's insert / read / update / delete + checkpoint-recover round-trip across the
    /// native-allocator switch (off and on), so the same behavior is validated on the managed backend and the
    /// direct-VM backend (index/log/frames off the GC heap). Complements the surface-specific fixtures
    /// (NativeAllocatorTests, NativeHashIndexTests, NativeSnapshotStressTests) with an end-to-end store path.
    ///
    /// The surfaces are a process-global installed once per test via <see cref="NativeAllocatorInitializer"/>, so the
    /// fixture is <see cref="NonParallelizableAttribute"/> and resets to managed in teardown. The direct-VM surfaces
    /// call the OS virtual-memory APIs directly and need no shipped native library.
    /// </summary>
    [TestFixture]
    [NonParallelizable]
    internal class NativeAllocatorStoreTests : TestBase
    {
        // Named cases so the test IDs read as (False) / (True).
        ...
        public void TearDown()
        {
            ...
            // These tests create native-backed stores whose direct-VM index/log blocks are freed lazily by the
        ...
        static void EnableOrSkip(bool useNative)
        {
            // The direct-VM surfaces call mmap/VirtualAlloc directly and need no shipped native library,
            // so it can be enabled unconditionally.
            _ = NativeAllocatorInitializer.Initialize(useNative);
        }
        ...
        public void InsertReadUpdateDelete(bool useNative)
        {
            EnableOrSkip(useNative);
            ...
            for (var i = 0; i < n; i++)
            {
                ClassicAssert.IsTrue(TryRead(i, out var v), $"[{useNative}] key {i} not found after insert");
            ...
            for (var i = 0; i < n; i++)
            {
                ClassicAssert.IsTrue(TryRead(i, out var v), $"[{useNative}] key {i} not found after update");
            ...
            for (var i = 0; i < n; i++)
            {
                ...
                if ((i & 1) == 0)
                    ClassicAssert.IsFalse(found, $"[{useNative}] deleted key {i} still present");
                else
                {
                    ClassicAssert.IsTrue(found, $"[{useNative}] key {i} missing after neighbor delete");
                    ClassicAssert.AreEqual(i + 1, v);
                }
            }
        }
        ...
        [Category(TsavoriteKVTestCategory)]
        [TestCaseSource(nameof(AllocatorModes))]
        public void CheckpointRecover(bool useNative)
        {
            EnableOrSkip(useNative);
            const int n = 10_000;
            Guid token;
            ...
            void VerifyAll(TsavoriteKV<SpanByteStoreFunctions, SpanByteAllocator<SpanByteStoreFunctions>> store)
            {
                ...
                for (var i = 0; i < n; i++)
                {
                    ...
                    ClassicAssert.IsTrue(status.Found, $"[{useNative}] key {i} not recovered");
            ...
            {
                UpsertAll(store);
                // Snapshot checkpoint: when native, this reads the direct-VM log pages.
                var initiated = store.TryInitiateFullCheckpoint(out token, CheckpointType.Snapshot);
                ClassicAssert.IsTrue(initiated, $"[{useNative}] checkpoint not initiated");
