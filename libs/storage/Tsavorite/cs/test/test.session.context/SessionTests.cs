...
namespace Tsavorite.test.Session
{
    ...
    [TestFixture]
    internal class SessionTests : TestBase
    {
        ...
        {
            using var session1 = store.NewSession<KeyStruct, InputStruct, OutputStruct, Empty, Functions>(new Functions());
            var bContext1 = session1.BasicContext;
            using var session2 = store.NewSession<KeyStruct, InputStruct, OutputStruct, Empty, Functions>(new Functions());
            ...
            status = bContext2.Read(key2, ref input, ref output, Empty.Default);
