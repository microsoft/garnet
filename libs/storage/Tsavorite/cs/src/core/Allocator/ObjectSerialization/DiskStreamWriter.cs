...
#pragma warning disable IDE0065 // Misplaced using directive
    ...
    internal unsafe partial class DiskStreamWriter : IStreamBuffer
    {
        ...
        internal CircularDiskWriteBuffer circularPageFlushBuffers;
        ...
        internal DiskWriteBuffer pageBuffer;
        ...
        DiskWriteBuffer prevPageBuffer;
...
#pragma warning disable IDE0290 // Use primary constructor
        public DiskStreamWriter(IDevice logDevice, CircularDiskWriteBuffer circularPageBuffers, IObjectSerializer<IHeapObject> valueObjectSerializer)
        {
        ...
        public void Write(in LogRecord logRecord)
        {
            ...
            Write(new ReadOnlySpan<byte>(&tempInfo, RecordInfo.Size));
            ...
            {
                Write(logRecord.AsReadOnlySpan().Slice(RecordInfo.Size));
                OnRecordComplete();
                return;
            }
            ...
            var usablePageSize = circularPageFlushBuffers.pageBufferSize;
        ...
        private int WriteLengthMetadata(in LogRecord logRecord, int keyLength, long valueLength, bool isChunked = false)
        {
            ...
            if (valueLengthPosition > PageEndPosition - sizeof(int))
            {
                ...
                var recordInfoPosition = pageBuffer.currentPosition - RecordInfo.Size;
                ...
                Write(new ReadOnlySpan<byte>(&logRecordInfo, RecordInfo.Size));
            }
        ...
        /// </summary>
        /// <returns></returns>
        bool FlushKeyInteriorIfNeeded(DiskWriteBuffer buffer)
        {
            if (!HasKeyInterior)
                return false;
        ...
        {
            _ = CalculatePageBreaks(overflowArray.Length, PageEndPosition - pageBuffer.currentPosition, circularPageFlushBuffers.pageBufferSize, out var pageBreakInfo);
        ...
        internal void WriteDirect(ReadOnlySpan<byte> data, byte[] targetArray = null)
        {
            ...
            _ = CalculatePageBreaks(data.Length, PageEndPosition - pageBuffer.currentPosition, circularPageFlushBuffers.pageBufferSize, out var pageBreakInfo);
        ...
        internal void WriteDirectInterior(ReadOnlySpan<byte> data, ref PageBreakInfo pageBreakInfo, byte[] targetArray)
        {
            ...
            for (var ii = 0; ii < pageBreakInfo.internalPageCount; ii++)
            {
                ...
                _ = (*(DiskPageHeader*)bufferPtr).Initialize((int)logDevice.SectorSize);
                ...
                data.Slice(dataOffset, copyLen).CopyTo(bufferSpan.Slice((int)logDevice.SectorSize + DiskPageHeader.Size, copyLen));
        ...
        /// <summary>Flush the buffer up to sector alignment below <see cref="DiskWriteBuffer.currentPosition"/>,
        ...
        private void FlushPartialBuffer(DiskWriteBuffer buffer)
        {
            if (buffer.currentPosition == 0 || buffer.flushedUntilPosition == buffer.currentPosition)
                return; // Nothing to flush

            Debug.Assert(RoundDown(buffer.currentPosition, (int)logDevice.SectorSize) == buffer.currentPosition,
            ...
            var flushLength = RoundDown(buffer.currentPosition, (int)logDevice.SectorSize) - buffer.flushedUntilPosition;
        ...
        /// Do not shift the buffer or update <see cref="DiskWriteBuffer.currentPosition"/>; subsequent operations will handle that</summary>
        private void FlushPartialBuffer(DiskWriteBuffer buffer, int startPosition, int endPosition)
        {
            if (endPosition - startPosition == 0)
                return; // Nothing to flush

            var flushLength = endPosition - startPosition;
            Debug.Assert(IsAligned(startPosition, (int)logDevice.SectorSize), $"startPosition {startPosition} should be sector-aligned and was not");
            Debug.Assert(IsAligned(endPosition, (int)logDevice.SectorSize), $"endPosition {endPosition} should be sector-aligned and was not");
            Debug.Assert(IsAligned(flushLength, (int)logDevice.SectorSize), $"flushLength {flushLength} should be sector-aligned and was not");
        ...
        private void FlushToDevice(DiskWriteBuffer buffer, ReadOnlySpan<byte> span, DiskWriteCallbackContext pageWriteCallbackContext = null)
        {
            Debug.Assert(IsAligned(span.Length, (int)logDevice.SectorSize), $"span.Length {span.Length} should be sector-aligned and was not");
            Debug.Assert(IsAligned((long)circularPageFlushBuffers.alignedNextDiskFlushAddress, (int)logDevice.SectorSize), $"alignedDeviceAddress {circularPageFlushBuffers.alignedNextDiskFlushAddress} should be sector-aligned and was not");
