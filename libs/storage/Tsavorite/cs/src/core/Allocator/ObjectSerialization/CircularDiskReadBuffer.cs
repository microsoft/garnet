...
#pragma warning disable IDE0065 // Misplaced using directive
    ...
    public class CircularDiskReadBuffer : IDisposable
    {
        ...
        internal CircularDiskReadBuffer(SectorAlignedBufferPool bufferPool, int pageBufferSize, int numPageBuffers, IDevice device, ILogger logger) 
        {
            ...
            buffers = new DiskReadBuffer[numPageBuffers];
            currentIndex = 0;
        }
        ...
        /// <summary>
        /// Prepare the <see cref="DiskReadBuffer"/> and local variables to read a chunk and be ready to compute the next chunk's sizes
        ...
        private DiskReadBuffer PrepareToReadChunk(int bufferIndex, int startPosition, long chunkLength, out int unalignedReadLength, out uint alignedReadLength)
        {
            var buffer = buffers[bufferIndex];
            if (buffer is null)
            {
                buffer = new DiskReadBuffer(bufferPool.Get(pageBufferSize), device, logger);
