...
namespace Tsavorite.core
{
    ...
    /// </summary>
    internal sealed unsafe class ObjectScanIterator<TStoreFunctions, TAllocator> : ScanIteratorBase<TAllocator>, ITsavoriteScanIterator, IPushScanIterator
        ...
        where TAllocator : IAllocator<TStoreFunctions>
    {
        ...
        internal ObjectScanIterator(TsavoriteKV<TStoreFunctions, TAllocator> store, AllocatorBase<TStoreFunctions, TAllocator> hlogBase,
            ...
            : base(beginAddress == 0 ? hlogBase.GetFirstValidLogicalAddressOnPage(0) : beginAddress, endAddress, diskScanBufferingMode, memScanBufferingMode,
                  includeClosedRecords, epoch, hlogBase.LogPageSizeBits, hlogBase._wrapper, logger: logger)
        {
            this.store = store;
        ...
        bool IPushScanIterator.BeginGetPrevInMemory<TKey>(TKey key, out LogRecord logRecord, out bool continueOnDisk)
        {
            while (true)
            {
                ...
                if (logRecord.Info.IsNull || !hlogBase.storeFunctions.KeysEqual(logRecord, key))
                {
                    epoch?.Suspend();
                    continue;
                }
