...
namespace Tsavorite.core
{
    ...
    public static class Devices
    {
        ...
        /// <param name="uringSqPoll">For DeviceType.Native on Linux with the io_uring backend: enable IORING_SETUP_SQPOLL so a kernel thread polls the submission queue (syscall-free submits). Multiple rings share one poll thread. Ignored for libaio / on Windows. Off by default.</param>
        ...
        public static IDevice CreateLogDevice(string logPath = null, DeviceType deviceType = DeviceType.Default, bool preallocateFile = false, bool deleteOnClose = false, long capacity = CAPACITY_UNSPECIFIED, bool recoverDevice = false, bool useIoCompletionPort = false, bool disableFileBuffering = true, bool readOnly = false, NativeStorageDevice.IoBackend ioBackend = NativeStorageDevice.IoBackend.Default, int numCompletionThreads = 1, long localMemorySegmentSize = 1L << 30, int localMemoryRingCapacity = 0, ILogger logger = null, int numIoContexts = 0, int queueDepth = 0, bool uringSqPoll = false, int uringSqPollIdleMs = 0)
        {
            ...
            {
                DeviceType.Native when RuntimeInformation.IsOSPlatform(OSPlatform.Linux) => new NativeStorageDevice(logPath, deleteOnClose, disableFileBuffering, capacity, numCompletionThreads: numCompletionThreads, ioBackend: ioBackend, logger: logger, numIoContexts: numIoContexts, queueDepth: queueDepth, uringSqPoll: uringSqPoll, uringSqPollIdleMs: uringSqPollIdleMs),
