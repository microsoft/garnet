    ...
    public sealed class TsavoriteLog : IDisposable
    {
        ...
        private TsavoriteLog(TsavoriteLogSettings logSettings, bool syncRecover, ILogger logger = null)
        {
            ...
            if (syncRecover)
            {
                try
                {
                    RecoverAsync(-1).AsTask().GetAwaiter().GetResult(); // TODO use BlockingWait after Garnet.common refactor, or convert to factory pattern with CreateAsync
                }
                catch { }
            }
        ...
        /// <summary>
        /// Asynchronously recover TsavoriteLog to the specific commit number, or latest if -1
        /// </summary>
