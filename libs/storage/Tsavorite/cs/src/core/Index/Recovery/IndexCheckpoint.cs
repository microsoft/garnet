    ...
    public partial class TsavoriteBase
    {
        ...
        internal unsafe void BeginMainIndexCheckpoint(int version, IDevice device, out ulong numBytesWritten, bool useReadCache = false, SkipReadCache skipReadCache = default, int throttleCheckpointFlushDelayMs = -1)
        {
            ...
            void FlushRunner()
            {
                try
                {
                    ...
                    for (int index = 0; index < numChunks; index++)
                    {
                        ...
                        if (!useReadCache)
                        {
                            ...
                            {
                                // Submission failed: no completion callback will fire, so release this chunk's unit here.
                ...
                {
                    // Release the issuance sentinel. Any chunk writes still in flight keep nativeIndexIoOutstanding
        ...
        private void AsyncPageFlushCallback(uint errorCode, uint numBytes, object context)
        {
            ...
            finally
            {
                ...
                // grow while this write was in flight are munmap'd. No-op for the managed hash index.
                EndNativeIndexCheckpointIo();
            }
        }
    }
