...
namespace Tsavorite.core
{
        ...
        where TAllocator : IAllocator<TKey, TValue, TStoreFunctions>
    {
        ...
        public ITsavoriteScanIterator<TKey, TValue> Iterate<TInput, TOutput, TContext, TSessionFunctions>(TSessionFunctions functions, long untilAddress = -1)
            where TSessionFunctions : ISessionFunctions<TKey, TValue, TInput, TOutput, TContext>
        {
            if (untilAddress == -1)
                untilAddress = Log.TailAddress;
            return new TsavoriteKVIterator<TKey, TValue, TInput, TOutput, TContext, TSessionFunctions, TStoreFunctions, TAllocator>(this, functions, untilAddress, loggerFactory: loggerFactory);
        }
        ...
        /// <param name="untilAddress">Report records until this address (tail by default)</param>
        /// <returns>Tsavorite iterator</returns>
        public bool Iterate<TInput, TOutput, TContext, Functions, TScanFunctions>(Functions functions, ref TScanFunctions scanFunctions, long untilAddress = -1)
            ...
            where TScanFunctions : IScanIteratorFunctions<TKey, TValue>
        {
            if (untilAddress == -1)
                untilAddress = Log.TailAddress;
            using TsavoriteKVIterator<TKey, TValue, TInput, TOutput, TContext, Functions, TStoreFunctions, TAllocator> iter = new(this, functions, untilAddress, loggerFactory: loggerFactory);
    ...
    internal sealed class TsavoriteKVIterator<TKey, TValue, TInput, TOutput, TContext, TSessionFunctions, TStoreFunctions, TAllocator> : ITsavoriteScanIterator<TKey, TValue>
        where TSessionFunctions : ISessionFunctions<TKey, TValue, TInput, TOutput, TContext>
        where TStoreFunctions : IStoreFunctions<TKey, TValue>
        where TAllocator : IAllocator<TKey, TValue, TStoreFunctions>
    {
        ...
        private readonly ClientSession<TKey, TValue, TInput, TOutput, TContext, TSessionFunctions, TStoreFunctions, TAllocator> tempKvSession;
        private readonly BasicContext<TKey, TValue, TInput, TOutput, TContext, TSessionFunctions, TStoreFunctions, TAllocator> tempbContext;
        ...
        public TsavoriteKVIterator(TsavoriteKV<TKey, TValue, TStoreFunctions, TAllocator> store, TSessionFunctions functions, long untilAddress, ILoggerFactory loggerFactory = null)
        {
            this.store = store;
            ...
            tempKvSession = tempKv.NewSession<TInput, TOutput, TContext, TSessionFunctions>(functions);
