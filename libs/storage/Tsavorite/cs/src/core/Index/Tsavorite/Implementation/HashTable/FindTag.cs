    ...
    internal unsafe partial struct HashTable
    {
        ...
        internal void FindOrCreateTag(ref HashEntryInfo hei, long BeginAddress)
        {
            ...
            while (true)
            {
                ...
                if (0 == Interlocked.CompareExchange(ref hei.bucket->bucket_entries[hei.slot], hei.entry.word, 0))
                {
                    ...
                    var orig_bucket = spine.state[version].tableAligned + masked_entry_word;  // TODO local var not used; use or change to byval param
        ...
        private bool FindTagOrFreeInternal(ref HashEntryInfo hei, long BeginAddress = 0)
        {
            ...
            do
            {
                ...
                hei.bucket = (HashBucket*)overflowBucketsAllocator.GetPhysicalAddress(target_entry_word & HashBucketEntry.kAddressMask);
            } while (true);
        }
