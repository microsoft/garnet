...
namespace Tsavorite.core
{
    ...
    /// </summary>
    internal sealed class FullCheckpointOrchestrationTask<TValue, TStoreFunctions, TAllocator> : ISynchronizationTask<TValue, TStoreFunctions, TAllocator>
    ...
    /// </summary>
    internal sealed class FullCheckpointStateMachine<TValue, TStoreFunctions, TAllocator> : HybridLogCheckpointStateMachine<TValue, TStoreFunctions, TAllocator>
        where TStoreFunctions : IStoreFunctions<TValue>
        where TAllocator : IAllocator<TValue, TStoreFunctions>
    {
        /// <summary>
        ...
        public FullCheckpointStateMachine(ISynchronizationTask<TValue, TStoreFunctions, TAllocator> checkpointBackend, long targetVersion = -1) : base(
            targetVersion, new VersionChangeTask<TValue, TStoreFunctions, TAllocator>(), new FullCheckpointOrchestrationTask<TValue, TStoreFunctions, TAllocator>(),
            new IndexSnapshotTask<TValue, TStoreFunctions, TAllocator>(), checkpointBackend)
        { }
