...
namespace Tsavorite.core
{
    ...
    /// pages, recovery frames) when the native allocator is enabled. Uses <c>mmap(MAP_ANONYMOUS)</c> on Linux
    ...
    /// surfaces. A general-purpose pooling allocator is deliberately not used here: for a handful of multi-GB
    /// mappings, segment/arena reuse can hand back dirty memory and offers no benefit over a plain OS mapping.
    /// </summary>
    internal static unsafe class DirectVirtualMemory
    {
        ...
        public static DirectVmBlock Allocate(long size, int alignment)
        {
            ...
            // small pages). A 2 MB alignment still satisfies any sector alignment (which is <= 2 MB). Gated on Linux
            // specifically (not merely non-Windows): MADV_HUGEPAGE is a Linux advice, and on macOS the 2 MB
            // over-alignment would only waste address space for no benefit.
            var useHugePages = OperatingSystem.IsLinux() && size >= HugePageSize;
