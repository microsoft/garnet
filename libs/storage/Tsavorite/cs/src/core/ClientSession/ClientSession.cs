...
namespace Tsavorite.core
{
        ...
        where TAllocator : IAllocator<TKey, TValue, TStoreFunctions>
    {
        ...
        /// <returns>True if Iteration completed; false if Iteration ended early due to one of the TScanIterator reader functions returning false</returns>
        public bool IterateLookup<TScanFunctions>(ref TScanFunctions scanFunctions, ref long cursor, long untilAddress = -1, bool validateCursor = false, bool resetCursor = true)
            where TScanFunctions : IScanIteratorFunctions<TKey, TValue>
        {
            if (untilAddress == -1)
                untilAddress = store.Log.TailAddress;
            return ScanCursor(ref cursor, count: long.MaxValue, scanFunctions, endAddress: untilAddress, validateCursor: validateCursor, resetCursor: resetCursor);
        }
