...
public:
    NativeDeviceImpl(const std::string& file,
        ...
        const char* uring_sqpoll_cpus = nullptr)
        : epoch_ { }
        ...
        // uring_sqpoll / uring_sqpoll_idle_ms / uring_sqpoll_cpus are io_uring-only (IORING_SETUP_SQPOLL,
        // the poll thread's idle window, and its optional per-ring CPU pin list); the libaio / ThreadPool
        // handlers accept and ignore them.
        , handler_{ 16 /*max threads*/, num_io_contexts < 1 ? 1 : num_io_contexts, max_events, uring_sqpoll, uring_sqpoll_idle_ms, uring_sqpoll_cpus }
