...
extern "C" {
	...
	/// syscall. Multiple rings share a single poll thread (ring 0 spawns it; the rest attach via
	/// IORING_SETUP_ATTACH_WQ). `uring_sqpoll_idle_ms` is that poll thread's idle-before-park window
