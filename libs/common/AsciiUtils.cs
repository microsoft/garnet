...
public static class AsciiUtils
{
    ...
    /// Check if two byte spans are equal, where right is an all-upper-case span, ignoring case if there are ASCII bytes.
    /// </summary>
    public static bool EqualsUpperCaseSpanIgnoringCase(this ReadOnlySpan<byte> left, ReadOnlySpan<byte> right, bool allowAllPrintableCharacters)
    {
        if (left.SequenceEqual(right))
            return true;
        if (left.Length != right.Length)
        ...
        for (var i = 0; i < left.Length; i++)
        {
            ...
            if ((allowAllPrintableCharacters) && ((b2 is < 65 and >= 32) || (b2 is < 127 and > 96)))
            {
                if (b1 == b2)
                    continue;
                return false;
