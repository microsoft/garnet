...
namespace GarnetRoaringBitmap
{
    ...
    public sealed class RoaringBitmapObject : CustomObjectBase
    {
            ...
            : base(type, ObjectOverhead)
        {
            ...
            HeapMemorySize = ObjectOverhead + bitmap.ByteSize;
        }
        ...
        {
            var sizeBefore = bitmap.ByteSize;
            var previous = bitmap.SetBit(offset, set);
            var sizeAfter = bitmap.ByteSize;
            HeapMemorySize += (sizeAfter - sizeBefore);
            return previous ? 1 : 0;
        }

        public int GetBit(uint offset) => bitmap.GetBit(offset) ? 1 : 0;

        public long BitCount() => bitmap.Cardinality;

        public long BitPos(int bit, uint from = 0) => bitmap.BitPos(bit != 0, from);
