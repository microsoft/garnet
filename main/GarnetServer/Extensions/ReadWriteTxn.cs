...
namespace Garnet
{
    ...
    sealed class ReadWriteTxn : CustomTransactionProcedure
    {
        ...
        {
            int offset = 0;
            api.GET(GetNextArg(ref parseState, ref offset), out var key1);
            if (key1.ReadOnlySpan.SequenceEqual("wrong_string"u8))
                return false;
            AddKey(GetNextArg(ref parseState, ref offset), LockType.Exclusive, false);
            AddKey(GetNextArg(ref parseState, ref offset), LockType.Exclusive, false);
            return true;
        }
        ...
        {
            var offset = 0;
            var key1 = GetNextArg(ref parseState, ref offset);
            var key2 = GetNextArg(ref parseState, ref offset);
            var key3 = GetNextArg(ref parseState, ref offset);
