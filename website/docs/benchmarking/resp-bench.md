...
```bash
dotnet run -c Release -f net10.0 -- --online --op-workload GET,SET --op-percent 50,50 -b 1  -t 1
```

For a run with 16 client sessions:

```bash
dotnet run -c Release -f net10.0 -- --online --op-workload GET,SET --op-percent 50,50 -b 1  -t 16
```

For a run using ZADD and ZCARD commands:

```bash
dotnet run -c Release -f net10.0 -- --online --op-workload ZADD,ZCARD --op-percent 50,50 -b 1  -t 1 --keylength 8 --client SERedis
```
...
```bash
dotnet run -c Release -f net10.0 -- --op GET -t 2,4,8,16 -b 512 --dbsize 10241024 --keylength 1024
```

```bash
dotnet run -c Release -f net10.0 -- --op ZADDCARD -t 64 -b 512 --dbsize 16777216 --keylength 512
```
**Where:**
...
```bash
dotnet run -c Release -f net10.0 -- --op MSET --dbsize 16777216 --keylength 512
```

Then execute the GET command in online mode:

```bash
dotnet run -c Release -f net10.0 -- --op-workload GET --online -b 1 --op-percent 100 -t 64 --client GarnetClientSession --itp 1024 --runtime -1 --dbsize 16777216 --keylength 512
```
