...
- `AddKey(PinnedSpanByte key, LockType type, StoreType storeType)` This method is used to add a specified key to the locking set. It takes three parameters: key (the key to be added), type (the type of lock to be applied), and storeType (indicating the store type the key belongs to).
- `RewindScratchBuffer(PinnedSpanByte slice)` This method is responsible for rewinding (popping) the last entry of the scratch buffer if it contains the given slice. It takes a `PinnedSpanByte` parameter and returns a boolean value indicating whether the rewind operation was successful.
- `CreateArgSlice(ReadOnlySpan<byte> bytes)` This method is used to create a `PinnedSpanByte` in the scratch buffer from a given ReadOnlySpan\<byte>. It takes a ReadOnlySpan\<byte> parameter representing the argument and returns a `PinnedSpanByte` object.
- `CreateArgSlice(string str)` This method is similar to the previous one, but it creates a `PinnedSpanByte` in UTF8 format from a given string. It takes a string parameter and returns a `PinnedSpanByte` object.
- `GetNextArg(ref CustomProcedureInput procInput, ref int offset)` This method is used to retrieve the next argument from the input at the specified offset. It takes the input and a reference to an int offset. It returns a `PinnedSpanByte` object representing the argument as a span. The method internally reads a pointer with a length header to extract the argument.
...
Registering the custom transaction is done on the server-side by calling the `NewTransactionProc(string name, Func<CustomTransactionProcedure> proc, RespCommandsInfo commandInfo = null, RespCommandDocs commandDocs = null)` method on the Garnet server object's `RegisterAPI` object with its name, a method that returns an instance of the custom transaction class, and optional RESP command info and docs.\
