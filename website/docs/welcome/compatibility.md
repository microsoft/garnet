...
8. **Streams are experimental and standalone-only.** Stream commands (`XADD`, `XREAD`, `XREADGROUP`, `XGROUP`, `XACK`, `XPENDING`, `XCLAIM`, `XAUTOCLAIM`, `XINFO`, etc.) are implemented for the single-node configuration only. They are **not supported in cluster mode** — slot routing, key migration, and replication of stream entries are not yet wired up. Consumer group state (groups, consumers, pending-entry lists) is persisted across restarts via the object-store checkpoint when `--stream-log-dir` is set, but is not replicated to replicas. See the [Stream commands reference](../commands/data-structures.md#stream) and the [Streams design doc](../dev/streams-design.md) for details.
