...
2. Garnet does not support Redis modules. Instead, it has its own C# based [extensibility](../extensions/overview.md) mechanisms
that are optimized for very high performance, thread scalability, safety, and ease of use.
3. Garnet has full-fledged and efficient support for Lua scripting. You can enable Lua with the `--lua` switch.
4. Garnet respects the FIFO ordering of request-responses. However, when used with larger-than-memory data, and with
...
5. When Garnet is used with the append-only-file (AOF) turned on, by default the server does not wait for commit before
