...
| **AofBarrierSpinUs** | ```--aof-barrier-spin-us``` | ```int``` | Integer in range:<br/>[-1, MaxValue] | How long a replay thread spins at the replay-align barrier before sleeping: -1 = spin forever, 0 = never spin, >0 = spin up to that many microseconds then sleep. |
