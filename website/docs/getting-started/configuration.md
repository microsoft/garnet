...
| **UseNativeAllocator** | ```--use-native-allocator``` | ```bool``` |  | Route large memory regions (log pages / hash index / recovery frames) through a native (off-managed-heap) direct-VM allocator (mmap/VirtualAlloc) instead of the GC heap. No native library is required. When enabled, size GCHeapHardLimit to leave headroom and monitor 'native_allocator_bytes' in INFO memory. |
| **UseLegacyBufferPool** | ```--use-legacy-buffer-pool``` | ```bool``` |  | Select the per-level ConcurrentQueue SectorAlignedBufferPool instead of the default origin-return (per-thread) pool. The origin-return pool returns each buffer to the thread that allocated it, so it scales with concurrent IO-completion threads under a per-pool byte budget. |
| **BufferPoolMemoryBudget** | ```--buffer-pool-memory-budget``` | ```string``` | Memory size | Per-pool managed byte budget for the default origin-return SectorAlignedBufferPool (ignored when --use-legacy-buffer-pool is set). Bounds total cacheable buffer bytes across all IO-completion threads, reserving 25% for small size-classes and 75% for large. Requests above the pooled ceiling, or beyond the budget, allocate on demand and free on return. Set to 0 to disable buffer caching entirely (applies to both pool backends). E.g. 1g, 8g. |
...
| **AofReplayDriftThreshold** | ```--aof-replay-drift-threshold``` | ```int``` | Integer in range:<br/>[-1, MaxValue] | Cross-sublog replay drift, in sequence-number units, tolerated on a replica before a replay-align barrier round is triggered. -1 disables the barrier. |
| **AofReplayDriftCheckFreq** | ```--aof-replay-drift-check-freq``` | ```int``` | Integer in range:<br/>[0, MaxValue] | How often the cross-sublog drift is re-checked during replay, as a multiple of aof-replay-drift-threshold: one scan per (this value x threshold) sequence-number window system-wide, rotated across replay threads (window index mod virtual sublog count), firing a replay-align round when the drift exceeds the threshold. 0 = readers about to wait are the only round source. |
| **AofReplayBarrierSpinUs** | ```--aof-replay-barrier-spin-us``` | ```int``` | Integer in range:<br/>[-1, MaxValue] | How long a replay thread spins at the replay-align barrier before sleeping: -1 = spin forever, 0 = never spin, >0 = spin up to that many microseconds then sleep. |
...
| **LatencyMonitorPrecision** | ```--latency-monitor-precision``` | ```int``` | Integer in range:<br/>[0, 5] | Significant decimal digits of value resolution kept by the latency histograms (0-5). Lowering this from 2 to 1 reduces latency monitor memory several-fold, at the cost of reporting percentiles to 10% rather than 1% resolution. |
...
| **NetworkBufferSize** | ```--network-buffer-size``` | ```string``` | Memory size | Size of the network send buffer, and initial size of the network receive buffer, held per connection (rounds down to power of 2). Total pinned network memory scales with this value times the connection count, so lower it for deployments with many concurrent connections. |
| **NetworkMaxReceiveBufferSize** | ```--network-max-receive-buffer-size``` | ```string``` | Memory size | Largest receive buffer size recycled by the network buffer pool (rounds down to power of 2). Larger payloads are still served, from a buffer allocated outside the pool and released once consumed. |
| **NetworkBufferPoolSize** | ```--network-buffer-pool-size``` | ```string``` | Memory size | Ceiling on the bytes retained by the shared network buffer pool for reuse across connections. |
| **NetworkBufferMemoryBudget** | ```--network-buffer-memory-budget``` | ```string``` | Memory size | Process-wide budget for the network buffers held by live connections, shared across all listeners. While connections are few this is slack and every connection gets the full ```--network-buffer-size```; once the budget divided by the live buffer count falls below that, the base size for new buffers adapts down toward ```--network-buffer-min-size``` so the total stays near the budget. Buffers still grow on demand beyond the base size. 0 disables adaptation, restoring unbounded per-connection sizing. |
| **NetworkBufferMinSize** | ```--network-buffer-min-size``` | ```string``` | Memory size | Smallest base size a receive buffer may be adapted down to when the network buffer memory budget is under pressure (rounds down to power of 2). |
| **NetworkSendBufferMinSize** | ```--network-send-buffer-min-size``` | ```string``` | Memory size | Smallest base size a send buffer may be adapted down to when the network buffer memory budget is under pressure (rounds down to power of 2). Higher than ```--network-buffer-min-size``` because an undersized send buffer pushes oversized responses onto a pooled-rental path. |
| **SessionScratchBufferMaxRetainedSize** | ```--session-scratch-buffer-max-retained-size``` | ```string``` | Memory size | Capacity each per-session scratch buffer may retain indefinitely. These pinned buffers grow to fit the largest request a session has served, so a ceiling stops one large command from permanently enlarging the session. Capacity above the ceiling is released at a periodic checkpoint; 0 disables shrinking. |
| **SessionParseStateMaxRetainedArgs** | ```--session-parse-state-max-retained-args``` | ```int``` | Integer in range:<br/>[0, MaxValue] | Argument capacity each session's RESP parse state may retain indefinitely. The parse state is sized by the argument count a client sends, so a ceiling stops one very wide command from permanently enlarging the session. 0 disables shrinking. |
...
| **AofReplayMaxLagBytes** | ```--aof-replay-max-lag-bytes``` | ```int``` | Integer in range:<br/>[-1, MaxValue] | Throttle ClusterAppendLog when replica.AOFTailAddress - ReplicationOffset > AofReplayMaxLagBytes. 0: Synchronous replay,  >=1: background replay with specified lag, -1: infinite lag |
| **AofSyncMaxLagBytes** | ```--aof-sync-max-lag-bytes``` | ```long``` | Integer in range:<br/>[-1, MaxValue] | Stall primary AOF appends when the replication lag exceeds this budget, divided evenly per sublog (each sublog stalls at an even 1/m share). -1: disabled, >=1: max lag in bytes. |
...
size the kernel IO path. Each controls exactly one dimension:

| Knob | Controls | Guidance |
| --- | --- | --- |
| `--device-io-backend` | `Libaio` (default) or `Uring` (io_uring) | Both reach the same peak on fast NVMe when sized correctly. |
| `--device-io-contexts` | Ring **count** (independent io_uring rings / libaio io_contexts) | **Critical for io_uring:** set at or above submitter concurrency (~your connection count) so each submitter owns a ring and submission is lock-free; too few rings serialize submitters on a per-ring lock and can cost ~3x. **libaio is largely indifferent** (its kernel io_context mutex is cheap), so it needs no tuning here. |
...
| `--device-completion-threads` | Number of background completion (drain) threads | Default 4 is sufficient on both backends; a single drainer convoys under high pending-read load. |
| `--device-throttle-limit` | Aggregate **in-flight** reads before backpressure | Software backpressure only — it bounds pinned read-buffer memory and must stay `≤ io-contexts × queue-depth`. Raise it for very high connection counts; the default saturates fast NVMe at typical thread counts. |

Mental model: `io-contexts` = number of submission queues, `queue-depth` = depth of each queue, so
`io-contexts × queue-depth` is the kernel capacity; `throttle-limit` is how much of that capacity the
allocator keeps in flight at once. The most common tuning mistake is running the **io_uring** backend
with too few rings — always set `--device-io-contexts` to cover your submitter concurrency for io_uring.
