...
| **AofReplayDriftCheckFreq** | ```--aof-replay-drift-check-freq``` | ```int``` | Integer in range:<br/>[0, MaxValue] | How often cross-sublog drift is re-checked during replay, as a multiple of aof-replay-drift-threshold. 0 = readers about to wait are the only round source. |
