...
Azure Cosmos DB Garnet Cache is currently in an expanded Private Preview. Please [register your subscription](./overview.md#register-your-subscription) to join the preview.
...
2. **Authentication**: Verify you've configured [role based access control](./security.md#authentication-and-access-control), which is required for data plane access. Role assignments can take several minutes to propagate after being created or modified.
...
3. **Network issues**: Test network connectivity between client and cache.
