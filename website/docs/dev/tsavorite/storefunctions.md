...
The Allocator definition supports two interfaces:
- `IAllocator`, the non-generic interface. This contains the performance-critical derived-Allocator methods called by `AllocatorBase` that we want to inline rather than virtcall (`AllocatePage`/`FreePage`/`OverflowPageCount`, `PopulateRecordSizeInfo`, and `GetPageOfAddress`). The struct wrapper `AllocatorBase._wrapper` implements `IAllocator`, so the call on `_wrapper` inlines, and then usually calls down to the derived `*AllocatorImpl` class implementation. Some members are implemented directly on the wrapper struct rather than calling through to the impl; for example `GetPageOfAddress` is specialized per allocator so that `TsavoriteLogAllocator` uses the full logical-address range while the main-store allocators mask off the read-cache bit (via `LogAddress`).
- `IAllocator<TStoreFunctions> : IAllocator`. This adds the inlined calls that need the `TStoreFunctions` type arg. `IAllocator` must propagate, so it is the base interface; the formerly separate `IAllocatorCallbacks` has been folded into `IAllocator`.
